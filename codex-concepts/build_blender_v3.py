"""Run: Blender --background --python build_blender_v3.py"""
import bpy, math
...
orb_objects = list(bpy.context.scene.objects)
# Revision 03: continuous anatomy, cupped ears and inset feline eyes.
fur = mat('Slate blue short fur', (.055,.205,.36))
lightfur = mat('Warm ivory muzzle', (.67,.73,.72))
stripe = mat('Tabby markings', (.014,.045,.079))
ear_skin = mat('Muted inner ear', (.43,.18,.21))
nose_mat = mat('Rose leather nose', (.25,.065,.08))
iris = mat('Jade iris', (.075,.43,.32))
...
for side in [-1,1]:
    ...
    parts=[arm,ball('Paw',(side*.34,1.34,-.61),(.29,.215,.31),lightfur)]
    for toe in [-1,0,1]:
        parts.append(ball('Toe',(side*.34+toe*.070,1.315,-.71),(.112,.145,.15),lightfur))
...
head_before = set(bpy.context.scene.objects)
fuse('Feline head sculpt',[
    ball('Skull',(0,2.17,-.39),(.96,.77,.74),fur),
    ball('Jaw',(0,1.99,-.49),(.61,.40,.53),fur),
    ball('Left cheek',(-.32,2.055,-.47),(.38,.37,.43),fur),
    ball('Right cheek',(.32,2.055,-.47),(.38,.37,.43),fur),
...
def cupped_ear(side,inset=False):
    ...
    if inset:
        a=(side*.215,2.36,-.493); b=(side*.451,2.34,-.379); c=(side*.467,2.64,-.358)
    else:
        a=(side*.145,2.325,-.47); b=(side*.515,2.27,-.31); c=(side*.49,2.715,-.305)
...
for side in [-1,1]:
    ...
    for j in range(48):
        t=2*math.pi*j/48
        u=math.cos(t)
        v=math.sin(t)*abs(math.sin(t))**.12
        x=center+.178*u
        y=2.185+.128*v+side*.009*u
        z=-.750+side*.22*(x-center)
        outline.append((x,y,z))
    # Convex iris-facing surface with its edge sunk into the skull.
    verts=[xyz((center,2.185,-.824))]+[xyz(v) for v in outline]
    ...
    for f in mesh.polygons: f.use_smooth=True
    tube('Dark eyelid edge',outline+[outline[0]],.007,rim_mat)
    ball('Soft oval pupil',(center,2.183,-.836),(.105,.192,.027),black)
    ball('Corneal glint',(center-.030,2.23,-.854),(.038,.044,.014),cream)
    ball('Small eye glint',(center+.023,2.146,-.852),(.013,.015,.009),cream)
    for row in range(3):
        root=(side*(.14+row*.021),2.015-row*.025,-.851)
        tapered('Whisker',[root,(side*.37,2.01-row*.038,-.89),
            (side*(.51+row*.025),2.06-row*.075,-.84)],.003,whisker_mat,[1,.7,.04])
        for col in range(2):
...
for side in [-1,1]:
    tapered('Forehead marking',[(side*.055,2.32,-.714),(side*.09,2.405,-.659),
        (side*.16,2.46,-.57)],.009,stripe,[.05,1,.08])
# Slight kitten proportions while preserving every facial detail together.
bpy.context.view_layer.update()
head_objects = set(bpy.context.scene.objects) - head_before
from mathutils import Matrix
pivot = Vector((0,-.39,2.16))
head_transform = Matrix.Translation(pivot) @ Matrix.Diagonal((1.055,1.035,1.035,1)) @ Matrix.Translation(-pivot)
for obj in head_objects: obj.matrix_world = head_transform @ obj.matrix_world
...
bpy.context.view_layer.update()
# Independent sibling roots. Parenting retains the authored riding pose.
asset = bpy.data.collections.new('03 Cat and orb'); bpy.context.scene.collection.children.link(asset)
rider_collection = bpy.data.collections.new('Rider'); asset.children.link(rider_collection)
orb_collection = bpy.data.collections.new('Orb'); asset.children.link(orb_collection)
rider_objects = [o for o in bpy.context.scene.objects if o not in orb_objects]
def make_root(name, location, collection):
    ...
    return obj
player_root = make_root('PlayerRoot', (0,0,0), asset)
rider_root = make_root('RiderRoot', (0,-.04,1.90), rider_collection)
orb_root = make_root('OrbRoot', (0,0,.78), orb_collection)
rider_root.parent = player_root; orb_root.parent = player_root
bpy.context.view_layer.update()
for objects, root, collection in [(rider_objects,rider_root,rider_collection),(orb_objects,orb_root,orb_collection)]:
    for obj in objects:
        world = obj.matrix_world.copy()
        for c in list(obj.users_collection): c.objects.unlink(obj)
        collection.objects.link(obj)
        obj.parent = root; obj.matrix_world = world
rider_root['purpose'] = 'Translate for jumping; rotate around local X for a whole-body flip. OrbRoot stays independent.'
orb_root['purpose'] = 'Independent orb center: rotate or translate without affecting rider.'
player_root['purpose'] = 'Move rider and orb together during normal travel.'
assert all(o.parent is rider_root for o in rider_objects)
assert all(o.parent is orb_root for o in orb_objects)
assert rider_root.parent is orb_root.parent is player_root
...
scene.render.image_settings.file_format = 'PNG'; scene.render.filepath = str(OUT/'blue-cat-v3-preview.png')
# Export only the concept, with curves converted by the exporter.
bpy.ops.object.select_all(action='DESELECT')
for o in asset.all_objects: o.select_set(True)
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'blue-cat-v3.blend'))
bpy.ops.export_scene.gltf(filepath=str(OUT/'blue-cat-v3.glb'), use_selection=True, export_apply=True)
# Optional separate files retain their own pivots; the combined file preserves the riding pose.
for filename, collection in [('cat-rider-v3.glb',rider_collection),('orb-v3.glb',orb_collection)]:
    bpy.ops.object.select_all(action='DESELECT')
